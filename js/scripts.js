...
window.onload = () => {
  // On écoute les cliques sur les touches
...
// On definit une variable pour indiquer si un point a déjà été utilisé
...
function gererTouches(event) {
  ...
  // On vérifie si on a l'évenement "keydown"
  ...
  if (parseFloat(touche) >= 0 || touche === ".") {
    // Vérification pour qu'on ne puisse pas mettre plusieurs points

    if (touche === "." && pointUtilise) {
      return;
    }
    ...
    // On met à jour la variable pointUtilise si un point est ajouté
    if (touche === ".") {
      pointUtilise = true;
    }
